...
public class SsoFilter implements Filter {
        ...
        if (Objects.isNull(authcationRpcService)) {
            try {
                authcationRpcService = (AuthcationRpcService) new HessianProxyFactory().create(AuthcationRpcService.class,
                        ssoServerUrl + "/hessian/authcationRpcService");
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        SessionUser sessionUser = Objects.isNull(req.getSession()) ? null : (SessionUser) session.getAttribute("sessionUser");
        ...
        if (StringUtils.isEmpty(token)) {
            ...
            if (!StringUtils.isEmpty(token = req.getParameter("token"))) {
                User user = authcationRpcService.findUserByToken(token);
            ...
            if (authcationRpcService.validate(token)) {
                chain.doFilter(request, response);
                return;
            }
